...
class db_operations():
    ...
    def single_attribute(self,query):
        try:
            ...
            print("Psssst Programmer, we got a Value Error with remove in single_attribute.")

    # function that updates the table with querys
    def update(self, query, dictionary):
        self.cursor.execute(query, dictionary)
        self.connection.commit()

    #function that deletes the chosen song
    def delete(self, query):
        self.cursor.execute(query)
        self.connection.commit()

#Only song name, album name, artist name, releasedate, and Explicit attributes can be modified).
    # function to return a single attribute values from table
    def multiple_attribute(self,query):
        self.cursor.execute(query)
        results = self.cursor.fetchall()
        #range_1 = {0,5,7,8,9,10,11,12}
        return results
